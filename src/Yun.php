...
class Yun
{
  ...
  {
    $this->Yfdate = $eightChar->getYfdate();
    $this->gender = $gender;
    // 阳
    $yang = 0 == $this->Yfdate->getYearGanIndexExact() % 2;
    // 男
    $man = 1 == $gender;
  ...
  {
    // 上节
    $prev = $this->Yfdate->getPrevJie();
    // 下节
    $next = $this->Yfdate->getNextJie();
    // 出生日期
    $current = $this->Yfdate->getSolar();
    // 阳男阴女顺推，阴男阳女逆推
    ...
    if (2 == $sect) {
      ...
      $endTimeZhiIndex = ($end->getHour() == 23) ? 11 : YfdateUtil::getTimeZhiIndex(substr($end->toYmdHms(), 11, 5));
      $startTimeZhiIndex = ($start->getHour() == 23) ? 11 : YfdateUtil::getTimeZhiIndex(substr($start->toYmdHms(), 11, 5));
      // 时辰差
