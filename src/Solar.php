...
class Solar
{
  ...
  public static function fromBaZiBySectAndBaseYear($yearGanZhi, $monthGanZhi, $dayGanZhi, $timeGanZhi, $sect, $baseYear)
  {
    ...
    $Yfdate = $today->getYfdate();
    $offsetYear = YfdateUtil::getJiaZiIndex($Yfdate->getYearInGanZhiExact()) - YfdateUtil::getJiaZiIndex($yearGanZhi);
    if ($offsetYear < 0) {
      $offsetYear = $offsetYear + 60;
    }
    $startYear = $Yfdate->getYear() - $offsetYear;
    $hour = 0;
    $timeZhi = substr($timeGanZhi, strlen($timeGanZhi) / 2);
    for ($i = 0, $j = count(YfdateUtil::$ZHI); $i < $j; $i++) {
      if (strcmp(YfdateUtil::$ZHI[$i], $timeZhi) === 0) {
        $hour = ($i - 1) * 2;
      }
    }
    while ($startYear >= $baseYear) {
      ...
      while ($counter < 15) {
        if ($year >= $baseYear) {
          ...
          if (strcmp($Yfdate->getYearInGanZhiExact(), $yearGanZhi) == 0 && strcmp($Yfdate->getMonthInGanZhiExact(), $monthGanZhi) == 0) {
            $found = true;
            break;
          }
        }
      ...
      if ($found) {
        ...
        while ($counter < 61) {
          $Yfdate = $solar->getYfdate();
          $dgz = (2 == $sect) ? $Yfdate->getDayInGanZhiExact2() : $Yfdate->getDayInGanZhiExact();
          if (strcmp($Yfdate->getYearInGanZhiExact(), $yearGanZhi) == 0 && strcmp($Yfdate->getMonthInGanZhiExact(), $monthGanZhi) == 0 && strcmp($dgz, $dayGanZhi) == 0 && strcmp($Yfdate->getTimeInGanZhi(), $timeGanZhi) == 0) {
            $l[] = $solar;
            break;
          }
          $solar = $solar->next(1);
  ...
  {
    return Yfdate::fromDate($this->calendar);
  }
