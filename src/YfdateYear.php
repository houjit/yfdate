...
class YfdateYear
{
  ...
  function __construct($YfdateYear)
  {
    YfdateYear::_initLeap();
  ...
  /**
   * 通过农历年初始化
   * @param int $YfdateYear 农历年
   * @return YfdateYear
   */
  public static function fromYear($YfdateYear)
  {
    $key = '_' . $YfdateYear;
    if (array_key_exists($key, YfdateYear::$CACHE)) {
      $obj = YfdateYear::$CACHE[$key];
    } else {
      $obj = new YfdateYear($YfdateYear);
      YfdateYear::$CACHE[$key] = $obj;
    }
    return $obj;
  }
  ...
  {
    if (empty(YfdateYear::$LEAP)) {
      for ($i = 0, $j = count(YfdateYear::$LEAP_11); $i < $j; $i++) {
        YfdateYear::$LEAP['_' . YfdateYear::$LEAP_11[$i]] = 13;
      }
      for ($i = 0, $j = count(YfdateYear::$LEAP_12); $i < $j; $i++) {
        YfdateYear::$LEAP['_' . YfdateYear::$LEAP_12[$i]] = 14;
      }
    }
  }
  ...
  /**
   * 获取农历月
   * @param int $YfdateMonth 月，1-12，闰月为负数，如闰2月为-2
   * @return YfdateMonth|null
   */
  public function getMonth($YfdateMonth)
  {
    foreach ($this->months as $m) {
      if ($m->getYear() == $this->year && $m->getMonth() == $YfdateMonth) {
        return $m;
      }
    }
    return null;
  ...
  private function compute()
  {
    ...
    $currentYearLeap = -1;
    if (array_key_exists('_' . $currentYear, YfdateYear::$LEAP)) {
      $currentYearLeap = YfdateYear::$LEAP['_' . $currentYear];
    } else {
      if ($hs[13] <= $jq[24]) {
        $i = 1;
    ...
    $prevYearLeap = -1;
    if (array_key_exists('_' . $prevYear, YfdateYear::$LEAP)) {
      $prevYearLeap = YfdateYear::$LEAP['_' . $prevYear] - 12;
    }
    ...
    for ($i = 0, $j = count($dayCounts); $i < $j; $i++) {
      ...
      $this->months[] = new YfdateMonth($y, $cm, $dayCounts[$i], $hs[$i] + Solar::$J2000);
      if (!$isNextLeap) {
        $m++;
      }
      if ($m == 13) {
  ...
  protected function getZaoByGan($index, $name)
  {
    ...
    $offset = $index - Solar::fromJulianDay($month->getFirstJulianDay())->getYfdate()->getDayGanIndex();
    if ($offset < 0) {
      $offset += 10;
    }
    return preg_replace('/几/', YfdateUtil::$NUMBER[$offset + 1], $name, 1);
  }
  ...
  protected function getZaoByZhi($index, $name)
  {
    ...
    $offset = $index - Solar::fromJulianDay($month->getFirstJulianDay())->getYfdate()->getDayZhiIndex();
    if ($offset < 0) {
      $offset += 12;
    }
    return preg_replace('/几/', YfdateUtil::$NUMBER[$offset + 1], $name, 1);
  ...
  {
    return YfdateYear::$YUAN[(int)(($this->year + 2696) / 60) % 3] . '元';
  }

  public function getYun()
  {
    return YfdateYear::$YUAN[(int)(($this->year + 2696) / 20) % 9] . '运';
  }

  public function getNineStar()
  {
    $index = YfdateUtil::getJiaZiIndex($this->getGanZhi()) + 1;
  ...
  {
    return YfdateUtil::$POSITION_TAI_SUI_YEAR[$this->zhiIndex];
  }

  public function getPositionTaiSuiDesc()
  {
    return YfdateUtil::$POSITION_DESC[$this->getPositionTaiSui()];
  }

  public function next($n)
  {
    return YfdateYear::fromYear($this->year + $n);
  }
}
